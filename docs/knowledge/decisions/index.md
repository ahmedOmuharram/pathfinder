...
- [A dead worker fails its turn by heartbeat, at five minutes of silence](a-dead-worker-fails-its-turn-by-heartbeat.md) - the sweep releases the jobs of a worker silent past 300 s as well as the jobs past the started-age timeout, Stop releases the same job on the spot, and a released turn closes its open tool calls first; a 60 s window was rejected because a live worker was measured 153 s behind, and the age timeout alone because it leaves a locked thread for an hour after an OOM kill
