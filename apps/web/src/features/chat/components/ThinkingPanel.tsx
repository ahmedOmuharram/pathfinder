...
export function ThinkingPanel(props: {
  ...
  const { isStreaming, activeToolCalls, lastToolCalls, subKaniCalls, subKaniStatus, reasoning, title } = props;
  ...
  return (
    <div className="flex justify-start animate-fade-in">
      <div className="w-[85%] shrink-0">
        <details open className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2">
        ...
        <div className="mt-2 space-y-3 text-[12px] text-slate-700">
          ...
          {activeToolCalls.length > 0 ? (
            ...
            <div className="flex items-center gap-2 rounded-md border border-slate-100 bg-slate-50 p-2 text-[11px] text-slate-500">
              ...
              <span>{reasoning ? "Waiting for tool calls…" : "Waiting for reasoning or tool calls…"}</span>
            </div>
          ) : null}
