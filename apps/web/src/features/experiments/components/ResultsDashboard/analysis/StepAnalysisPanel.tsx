...
import { useAsyncAction, loggedCatch } from "@/lib/utils/asyncAction";
...
export function StepAnalysisPanel({ experimentId }: StepAnalysisPanelProps) {
  ...
  const { run: loadTypes, error, loading } = useAsyncAction();
  const {
    run: runAnalysis,
    error: runError,
    loading: running,
  ...
  useEffect(() => {
    ...
    void loadTypes(async () => {
      const { analysisTypes: types } = await getExperimentAnalysisTypes(experimentId);
  ...
  }, [experimentId, selectedType, paramValues, runAnalysis]);
