...
/** Steps not reachable from ``rootStepId`` via primary/secondary input
 * chains. These end up invisible in the spine renderer and break
 * ``serializeStrategyAst`` (multiple-roots), so the panel needs to surface
 * them so the user can prune them. */
