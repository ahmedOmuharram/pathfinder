...
export function LabelEditor({
  ...
  const setLabelMutation = useMutation({
    mutationFn: async (label: string) => {
      await client({ method: "put", url: `/api/v1/conversations/${chatId}/labels/${checkpointId}`, data: { label } });
    },
  });
  const deleteLabelMutation = useMutation({
    mutationFn: async () => {
      await client({ method: "delete", url: `/api/v1/conversations/${chatId}/labels/${checkpointId}` });
    },
  });
