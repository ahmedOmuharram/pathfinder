...
def vcr_config():
    ...
    """
    return {
        "cassette_library_dir": "src/pathfinder/tests/cassettes",
...
async def create_taxon_step(api: StrategyAPI) -> WDKIdentifier:
    ...
    """
    from pathfinder.domain.parameters.vocab_utils import (  # noqa: PLC0415
        collect_leaf_terms,
    )
...
async def discover_gene_ids(api: StrategyAPI, limit: int = 5) -> list[str]:
    ...
    """
    from pathfinder.services.wdk.step_results import (  # noqa: PLC0415
        StepResultsService,
    )
