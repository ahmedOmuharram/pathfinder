...
async def test_deleted_gene_set_memory_does_not_resurrect(
    ...
    state = PipelineState(
        ...
        mode="strategy",
        user_prompt="q",
        created_gene_set_ids=["gs-delete-me"],
    )
    ...
    async with lifespan_memory_store(database_url) as raw:
        ...
        written_first = await auto_write_memories(
            store=store, tombstones=tombstones, state=state
        )
...
async def test_tombstone_is_kind_scoped_not_global(
    ...
    async with lifespan_memory_store(database_url) as raw:
        ...
        gs_state = PipelineState(
            ...
            mode="strategy",
            user_prompt="q",
            created_gene_set_ids=["shared-id"],
        )
        await auto_write_memories(store=store, tombstones=tombstones, state=gs_state)
