...
uv run pytest src/pathfinder/tests/integration/ -v --record-mode=all -p no:xdist
```
...
cd apps/api
WDK_AUTH_EMAIL=your@email.com WDK_AUTH_PASSWORD=yourpassword \
uv run pytest src/pathfinder/tests/integration/test_your_file.py -v --record-mode=new_episodes
```
...
cd apps/api
WDK_AUTH_EMAIL=your@email.com WDK_AUTH_PASSWORD=yourpassword \
uv run pytest src/pathfinder/tests/integration/test_your_new_file.py -v --record-mode=new_episodes
```
...
from pathfinder.tests.conftest import create_taxon_step, materialize_step
