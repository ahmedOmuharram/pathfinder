"""AI tools for experiment strategy refinement (pydantic-ai standalone functions).
...
async def refine_with_search(
    ...
    Args:
        search_name: WDK search name for the new step.
        parameters: Search parameters as key-value pairs.
        operator: Boolean operator: INTERSECT, UNION, MINUS.
    """
...
async def refine_with_gene_ids(
    ...
    Args:
        gene_ids: List of gene IDs to filter/combine with.
        operator: Boolean operator: INTERSECT, UNION, MINUS.
    """
    exp = await _get_experiment(ctx)
    if not exp or not exp.wdk_strategy_id or not exp.wdk_step_id:
...
async def re_evaluate_controls(
    ctx: RunContext[WorkbenchDeps],
) -> JSONObject:
