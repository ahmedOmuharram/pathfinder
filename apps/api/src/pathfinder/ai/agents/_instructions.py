...
def pinned_problem_frame(ctx: RunContext[AgentDeps]) -> str | None:
    ...
    if frame.strategy_sketch:
        lines.extend(["", "### Strategy Sketch (loose outline, not a plan)"])
        for node in frame.strategy_sketch:
            ref = (
                f" inputs={node.inputs} op={node.operator}"
    ...
    if plan.status == PlanStatus.APPROVED:
        lines.extend(
            [
                ...
                "Return your typed delta and let the Lead handle execution.",
            ],
        )
    return "\n".join(lines)
