...
async def test_sync_wdk_creates_projections_from_summary_only(
    ...
    items = [
        ...
        strategy_list_item(
            ...
            name="Strategy B",
            estimated_size=75,
            leaf_and_transform_step_count=1,
        ),
        strategy_list_item(
            strategy_id=300,
            name="Strategy C",
...
async def test_get_strategy_lazy_loads_detail_for_summary_only_projection(
    ...
    # Step 2: Mock the WDK detail endpoint for lazy fetch
    wdk_detail = {
        "strategyId": 500,
        "name": "Lazy Load Test",
        "description": "",
    ...
    # Mock search details for parameter normalization (may be called during lazy fetch)
    wdk_respx.post(
    ...
    # Step 3: GET the strategy — should trigger lazy detail fetch
...
async def test_lazy_fetch_multi_step_populates_all_counts(
    ...
    # Step 2: Mock WDK detail with a 3-step strategy (2 leaves + 1 combine)
    wdk_detail = {
        "strategyId": 600,
    ...
    # Mock search details for param normalization
    wdk_respx.post(
        url__regex=r".*/record-types/.*/searches/.*/refreshed-dependent-params"
    ).respond(200, json={})
    wdk_respx.get(url__regex=r".*/record-types/.*/searches/.*").respond(200, json={})

    # Step 3: GET the strategy — lazy fetch should populate counts
    get_resp = await authed_client.get(f"/api/v1/strategies/{strategy_id}")
    assert get_resp.status_code == 200
