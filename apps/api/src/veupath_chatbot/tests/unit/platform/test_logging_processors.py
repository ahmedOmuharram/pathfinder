...
from veupath_chatbot.platform.logging import add_app_context, add_otel_context
...
def test_add_otel_context_with_active_span():
    ...
    with patch("veupath_chatbot.platform.logging.trace.get_current_span", return_value=mock_span):
        event_dict: dict[str, object] = {"event": "test"}
        result = add_otel_context(logging.getLogger(), "", event_dict)
