...
Covers _format_param_info behavior with phyletic structural params
...
def _names(result: JSONArray) -> list[str]:
    """Extract parameter names from _format_param_info output."""
    return [p["name"] for p in result if isinstance(p, dict)]


def _find_param(result: JSONArray, name: str) -> dict[str, Any]:
    """Find a parameter by name in _format_param_info output."""
    for p in result:
...
class TestFormatParamInfoPhyleticFiltering:
    """_format_param_info strips phyletic structural params from AI output."""

    def test_phyletic_indent_map_excluded(self) -> None:
        result = _format_param_info(PHYLETIC_PARAMS)
        names = _names(result)
...
class TestFormatParamInfoProfilePatternHelp:
    """_format_param_info enriches profile_pattern help text."""
    ...
    def test_profile_pattern_help_contains_exclude_example(self) -> None:
        result = _format_param_info(PHYLETIC_PARAMS)
        pp = _find_param(result, "profile_pattern")
        assert "CODE=0T" in pp["help"]


class TestFormatParamInfoNonPhyletic:
    """Non-phyletic params pass through _format_param_info unchanged."""
    ...
    def test_non_phyletic_params_unaffected(self) -> None:
        ...
        result = _format_param_info(normal_params)
        names = _names(result)
    ...
    def test_non_phyletic_help_preserved(self) -> None:
        ...
        result = _format_param_info(params)
        assert result[0]["help"] == "Select organism"

    def test_empty_input_returns_empty(self) -> None:
        assert _format_param_info([]) == []

    def test_skips_non_dict_entries(self) -> None:
        result = _format_param_info(["not_a_dict", 42, None])
        assert result == []

    def test_skips_entries_without_name(self) -> None:
        result = _format_param_info([{"type": "string", "help": "no name"}])
        assert result == []
