...
# ---------------------------------------------------------------------------
# WDK push (best-effort, non-fatal)
# ---------------------------------------------------------------------------
...
async def _push_combine_step(
    ...
    """Push a combine step to WDK. Returns the WDK step ID or None if inputs are missing."""
    primary_wdk_id = (
...
async def _push_transform_step(
    ...
    """Push a transform step to WDK. Returns the WDK step ID or None if input is missing."""
    input_wdk_id = (
        graph.wdk_step_ids.get(step.primary_input.id) if step.primary_input else None
    )
