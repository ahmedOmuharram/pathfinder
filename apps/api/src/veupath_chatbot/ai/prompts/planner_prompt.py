...
from veupath_chatbot.ai.prompts.loader import load_planner_prompt
...
def build_planner_system_prompt(
    *,
    ...
    site_context = (
        f"\n\n## Current Session\n"
        f"You are currently working with the **{site_id}** database. "
        "Use this site for all searches and operations unless the user asks "
        "to switch sites."
    )
    draft_context = ""
    if delegation_draft_artifact:
        parameters = delegation_draft_artifact.get("parameters")
        if isinstance(parameters, dict):
            parameters_dict = as_json_object(parameters)
            goal = parameters_dict.get("delegationGoal")
            plan = parameters_dict.get("delegationPlan")
        else:
            goal = None
            plan = None
        ...
        if (goal and str(goal).strip()) or plan:
            ...
            draft_context = (
                ...
                "A delegation plan draft already exists for this plan session. "
                "Treat it as the **current source of truth**.\n"
                "- Do **not** restart from scratch unless the user asks.\n"
                "- Before running catalog/search tools, review this draft and "
                "only fetch missing metadata needed to update it.\n"
                "\n"
    ...
    if selected_nodes:
        ...
        node_context = (
            ...
            "Use their IDs when referencing existing steps; do not modify "
            "the graph unless asked.\n"
            f"```json\n{selected_nodes_json}\n```"
        )
    mention_block = ""
    ...
    return base_prompt + site_context + draft_context + node_context + mention_block
